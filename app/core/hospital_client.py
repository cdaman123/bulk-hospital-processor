...
class HospitalDirectoryClient:
    ...
    def create_hospital(self, name: str, address: str, phone: str | None, batch_id: str) -> dict:
        ...
        with httpx.Client(timeout=self.timeout) as client:
            try:
                ...
                logger.error(f"HTTP error {e.response.status_code} creating hospital: {e.response.text}")
    ...
    def activate_batch(self, batch_id: str) -> None:
        ...
        with httpx.Client(timeout=self.timeout) as client:
            try:
                ...
                logger.error(f"HTTP error {e.response.status_code} activating batch {batch_id}: {e.response.text}")
                raise ExternalAPIError(
                    f"API Error: {e.response.status_code}",
                    status_code=e.response.status_code,
                    response_body=e.response.text
